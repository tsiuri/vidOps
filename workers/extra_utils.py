...
from services import get_extra_utils_service

logger = logging.getLogger(__name__)


class ExtraUtilsWorker:
    """
    Worker that processes `job_type='extra_utils'` jobs using ExtraUtilsService.
    """

    def __init__(self):
        self.config = load_config()
        self.worker_type = "extra_utils"
    ...
    def _handle_signal(self, signum, frame):
        logger.info("Extra-utils worker %s received signal %s", self.worker_id, signum)
        self._shutdown_requested = True

    def run(self):
        logger.info("Extra-utils worker %s starting on %s", self.worker_id, self.machine_alias)
            ...
            while not self._shutdown_requested:
                if self._process_single_job():
                    ...
                    if self.max_jobs > 0 and processed >= self.max_jobs:
                        logger.info("Extra-utils worker processed %d jobs; stopping per max_jobs", processed)
                        break
                    continue
    ...
    def _heartbeat(self):
        now = time.time()
        if now - self.last_heartbeat >= self.config.workers.heartbeat_interval:
    ...
    def _process_single_job(self) -> bool:
        ...
        logger.info("Processing extra-utils job %s", job.job_id)
