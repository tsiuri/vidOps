...
from vidops.services import get_stitching_service

logger = logging.getLogger(__name__)

class StitchWorker:
    """
    A worker process that claims and processes stitching jobs from the database.
    """
    
    def __init__(self):
        self.config = load_config()
        self.worker_id = f"{self.config.workers.machine_alias}-stitching-{os.getpid()}"
    ...
    def _register_worker(self):
        ...
        worker_model = Worker(
            ...
            capabilities=["ffmpeg_stitching"] # Placeholder
        )
        self.worker_repo.register(worker_model)
        logger.info(f"StitchWorker '{self.worker_id}' registered as {WorkerStatus.IDLE.value}.")
    ...
    def _process_single_job(self):
        ...
        if job:
            ...
            logger.info(f"StitchWorker '{self.worker_id}' claimed job '{job.job_id}'.")
            self._update_status(WorkerStatus.BUSY, job.job_id)
            try:
                self.stitching_service.process_job(job)
            ...
            logger.debug(f"StitchWorker '{self.worker_id}' found no pending jobs.")
            self._update_status(WorkerStatus.IDLE)
    ...
    def _get_self_worker_model(self) -> Worker:
        ...
        return Worker(
            ...
            pid=self.pid,
            hostname=self.hostname,
            capabilities=["ffmpeg_stitching"] # Dummy capabilities for now
        )

    def run(self):
        """Main loop for the worker."""
        logger.info(f"Starting StitchWorker '{self.worker_id}'...")
        self.running = True
        ...
        logger.info(f"StitchWorker '{self.worker_id}' shutting down.")
