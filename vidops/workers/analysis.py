...
class AnalysisWorker:
    """
    A worker process that claims and processes analysis jobs from the database.
    """
    
    def __init__(self):
        self.config = load_config()
        self.worker_id = f"{self.config.workers.machine_alias}-analysis-{os.getpid()}"
        self.worker_type = "analysis"
    ...
    def _register_worker(self):
        ...
        logger.info(f"AnalysisWorker '{self.worker_id}' registered as {WorkerStatus.IDLE.value}.")
    ...
    def _process_single_job(self):
        ...
        job = self.job_repo.claim_next(
            ...
            lease_duration=timedelta(minutes=self.config.workers.heartbeat_interval * 2),
            job_types=[self.worker_type],
        )
        
        if job:
            self.current_job_id = job.job_id
            logger.info(f"AnalysisWorker '{self.worker_id}' claimed job '{job.job_id}' (YTID: {job.ytid}).")
            self._update_status(WorkerStatus.BUSY, job.job_id)
            try:
                self.analysis_service.process_job(job)
            ...
            logger.debug(f"AnalysisWorker '{self.worker_id}' found no pending jobs.")
            self._update_status(WorkerStatus.IDLE)
    ...
    def run(self):
        ...
        logger.info(f"Starting AnalysisWorker '{self.worker_id}'...")
        self.running = True
        ...
        logger.info(f"AnalysisWorker '{self.worker_id}' shutting down.")
