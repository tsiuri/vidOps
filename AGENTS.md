...
2026-01-01 update: Transcription no longer shells to `workspace.sh`; it runs native faster-whisper (word timestamps), writes VTT/words locally, and persists via FilesystemCache/DB ingestion (Windows-safe).
2026-01-01 update: Clipping (clips cut-local/cut-net) now uses native ffmpeg/yt-dlp with legacy filenames/paths preserved under `generated/hits/<run>/`, no `workspace.sh` dependency.
...
- VRAM-based analysis scheduling replaced the legacy capabilities system (removed 2025-12-31 via Migration 007). Tasks store `required_vram_gb` + `model_profile_id`, workers advertise available VRAM, and model profiles live in `analysis_model_profiles` (numeric `id` keys only). The `required_capabilities` and `capabilities` columns have been dropped from the database.
- Model profiles are seeded via `scripts/db/seed_model_profiles.py`, which serves each Ollama model with default settings, measures VRAM usage via `ollama ps`, and inserts rows into `analysis_model_profiles`. Run with `--dry-run` to preview, `--models MODEL1,MODEL2` to seed specific models, or without flags to seed all available Ollama models. The script extracts **only VRAM-affecting parameters** (`num_ctx`, `num_batch`, `num_keep`) from each model's Modelfile and stores them in the `options` JSONB column. Inference parameters (temperature, top_p, stop tokens, etc.) belong in the analysis config, not the model profile.
- **How VRAM-based scheduling works:** See `docs/VRAM_BASED_SCHEDULING.md` for comprehensive documentation. In brief: (1) Model profiles store VRAM requirements and memory-affecting parameters (num_ctx, num_batch). (2) Task creation resolves profiles and sets required_vram_gb per pass. (3) Workers advertise available VRAM. (4) Claim query atomically matches tasks where required_vram_gb <= worker_vram_gb AND profile_id matches. (5) Workers pass profile options to Ollama for consistent memory allocation. Inference parameters (temperature, stop tokens) come from analysis config, not profiles. Set `analysis.default_model_profile_id` and `analysis.default_vram_gb` in `config.yaml` or use `--model-profile-id` / `--vram-gb` flags when starting workers.
- Worker heartbeats now run in a shared background thread across workers and touch active jobs to keep `updated_at` fresh; analysis-distributed workers now register in the `workers` table.
