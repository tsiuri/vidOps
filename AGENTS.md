...
- VRAM-based analysis scheduling now supersedes the legacy capabilities system. Tasks store `required_vram_gb` + `model_profile_id`, workers advertise available VRAM, and model profiles live in `analysis_model_profiles` (numeric `id` keys only). Capabilities remain in the DB for potential future reimplementation but are not used for task claiming.
