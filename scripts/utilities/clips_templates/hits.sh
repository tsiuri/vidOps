...
Sources
  auto (default): per video ID, prefer *.words.tsv, then *.words.yt.tsv, then *.tslog.txt
...
Locations (where files are searched)
  words: searched recursively under generated/ and the project root.
         Matches files named words.tsv, *.words.tsv, and *.words.yt.tsv.
  tslog: searched recursively under generated/ and the project root.
         Matches files named *.tslog.txt.
  auto:  discovers video IDs from media files in pull/ and entries in generated/
         (top level); for each ID, searches generated/ then the project root
         recursively using the source priority above. Filenames must contain
         the 11‑char YouTube ID to associate logs with the correct video.
...
Examples
  # Exact spans from words with padding
  PAD_START=0.25 PAD_END=0.50 \\
    ./clips.sh hits --words --exact -q "hasan,react" -o results/wanted.tsv
E
        return 0;;
...
def discover_video_ids():
    ...
    pull_dir = os.path.join(project_root, 'pull')
    if os.path.isdir(pull_dir):
        for fname in os.listdir(pull_dir):
...
def find_best_source_for_video(vid):
    ...
    for root in [generated_dir, project_root]:
    ...
    # Priority 2: words.yt.tsv (generated by convert-captions)
    for root in [generated_dir, project_root]:
        if not os.path.isdir(root):
            continue
        for dirpath, _, files in os.walk(root):
    ...
    # Priority 3: tslog.txt in generated/
    for root in [generated_dir, project_root]:
        if not os.path.isdir(root):
            continue
        for dirpath, _, files in os.walk(root):
