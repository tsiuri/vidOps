...
class TranscriptChunker:
    ...
    def chunk(self, text: str) -> List[Dict[str, Any]]:
        """Split text into overlapping chunks by sentences with robust fallback.

        Notes:
        - Some VTT transcripts lack punctuation, yielding "sentences" that are extremely long.
          We first split by punctuation; any sentence longer than chunk_size is further split
          into sub-sentences of length <= chunk_size words to avoid giant chunks.
        - Overlap is applied in words from the end of the previous chunk.
        """
        # 1) Split into sentences using punctuation as delimiters
        raw = re.split(r'([.!?]+\s+)', text)

        # 2) Build sentence units, ensuring each unit <= chunk_size words
        units: List[str] = []
        for i in range(0, len(raw), 2):
            ...
            if len(w) <= self.chunk_size:
                ...
                # Fallback: split long sentence into word-sized slices
                for j in range(0, len(w), self.chunk_size):
                    units.append(' '.join(w[j:j + self.chunk_size]))

        # If punctuation-based splitting failed badly (very few units), fallback to word slicing
        if len(units) <= 1:
            words = text.split()
            if not words:
                return []
            units = [' '.join(words[j:j + self.chunk_size]) for j in range(0, len(words), self.chunk_size)]
        ...
        def flush_chunk():
            nonlocal current_words, chunk_id
            ...
            if self.overlap > 0 and len(current_words) > 0:
        ...
        for u in units:
            u_words = u.split()
            # If this unit would push beyond chunk_size and we have content, flush
            if current_words and (len(current_words) + len(u_words) > self.chunk_size):
                flush_chunk()
            current_words.extend(u_words)

            # Extremely long unit could still exceed chunk_size if current was empty
...
class OllamaAnalyzer:
    ...
    def __init__(self, model: str = "llama3.2", base_url: str = "http://localhost:11434",
                 options: Dict[str, Any] | None = None, custom_request: str | None = None,
    ...
    def analyze_chunk(self, chunk_text: str, chunk_id: int) -> Dict[str, Any]:
        ...
        if self.category_suggestions:
            cat_hint = (
                "\nWhen assigning categories, prefer these canonical tags if relevant:\n"
                + ", ".join(self.category_suggestions[:50])
                + "\n"
            )
        prompt = f"""Analyze this transcript segment and extract structured information.{extra}
{cat_hint}
...
Return only the JSON object."""
        ...
        try:
            ...
            if self.log_mode == "verbose":
                print(f"\n[chunk {chunk_id}] Raw LLM response:\n{response_text}\n", flush=True)
    ...
    def summarize(self, final_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        extra = f"\nAdditional instruction for this run: {self.custom_request}\n" if self.custom_request else "\n"
        prompt = f"""
Using the provided structured digest, write multiple summaries in strict JSON.{extra}
...
- bullet_points: 6-10 items; each MUST be a factual actor–action–object statement tied to a concrete issue.
  Format each bullet as: "Actor — action — object (issue)". Keep <= 18 words.
- political_overview: 6-12 sentences. Use this labeled outline, one or more sentences per label, in order:
  Context: [named events/dates/locations].
  Actors: [named people/orgs/countries and roles].
  Issues: [explicit list of political topics].
  Claims: [who claims what].
  Evidence: [direct quotes or concrete references from transcript only].
  Policy: [policies/votes/sanctions/treaties or procedural steps].
  Geo: [alliances/blocks/dependencies/sanctions/leverage].
  Impacts: [domestic/economic/social/civil-liberties effects].
  Uncertainties: [disputed facts or unknowns].
  SpeakerOpinion: [speaker's explicit opinions or stated positions].
- controversies: 2-4 sentences capturing conflicts/drama/disputes/allegations, naming parties and why it matters.
- issues: comprehensive list of political issues/topics referenced (e.g., "ukraine war", "immigration policy"),
  lowercase phrases, 1–5 words each, deduplicated; only include if actually referenced.
- Style constraints: never address the reader as "you" or use first-person plural "we". Refer to the speaker
  in third person as "the speaker" when needed. Do not imply reader involvement.
...
"""
    ...
    def summarize_speaker(self, final_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        topics = ", ".join([t.get('topic','') for t in data.get('topics', [])][:10])
        points = "\n- ".join(data.get('key_points', [])[:10])
        quotes = "\n> ".join(data.get('notable_quotes', [])[:8])

        extra = f"\nAdditional instruction for this run: {self.custom_request}\n" if self.custom_request else "\n"
        prompt = f"""
Focus ONLY on the speaker's personal content in strict JSON.{extra}
...
Rules:
- speaker_overview: 4–8 sentences about persona/self-positioning, values, admissions, calls to action.
- personal_themes: 5–12 recurring themes, lowercased concise phrases (1–5 words).
- noteworthy_statements: 3–10 short direct quotes (or concise paraphrases) that are personally revealing or controversial.
- personal_conflicts: 0–8 items naming conflicts between the speaker and another creator/person; name both sides and gist.
Only use information present in the transcript context; no outside knowledge.
 Style constraints: never address the reader as "you" or use first-person plural "we". Refer to the speaker
 as "the speaker"; do not imply reader involvement.
...
"""
        ...
        try:
            ...
            # Normalize
            speaker_overview = str(raw.get('speaker_overview','')).strip()
            def to_list(val):
                if isinstance(val, list): return [str(x).strip() for x in val if str(x).strip()]
                if isinstance(val, str): return [s.strip() for s in re.split(r"[\n,;|]+", val) if s.strip()]
                return []
            personal_themes = [s.lower() for s in to_list(raw.get('personal_themes', []))][:20]
            noteworthy = to_list(raw.get('noteworthy_statements', []))[:12]
            conflicts = to_list(raw.get('personal_conflicts', []))[:12]
            return {
                "speaker_overview": speaker_overview,
                "personal_themes": personal_themes,
                "noteworthy_statements": noteworthy,
                "personal_conflicts": conflicts
            }
        except requests.exceptions.RequestException:
            return {
                "speaker_overview": "",
    ...
    def _normalize_summaries(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(bullets_raw, str):
            ...
            bullets = []
        # political/controversial fields
        pol = str(raw.get("political_overview", "")).strip()
        cont = str(raw.get("controversies", "")).strip()
        issues_raw = raw.get("issues", [])
        if isinstance(issues_raw, str):
            issues = [t.strip() for t in re.split(r"[\n,;|]+", issues_raw) if t.strip()]
        elif isinstance(issues_raw, list):
            issues = [str(t).strip() for t in issues_raw if str(t).strip()]
        else:
            issues = []
        # normalize issues: lowercase, dedupe, limit length
        norm_issues = []
        seen_i = set()
        for t in issues:
            t2 = re.sub(r"\s+", " ", t.lower()).strip()
            if t2 and t2 not in seen_i:
                norm_issues.append(t2[:60])
                seen_i.add(t2)
        # parse noteworthy/conflicts arrays
        def to_list(val):
            ...
            return []
        noteworthy = to_list(raw.get("noteworthy_statements", []))
        conflicts = to_list(raw.get("personal_conflicts", []))
        def dedupe_keep(seq):
            out, seen = [], set()
            for s in seq:
                if s not in seen:
        ...
        return {
            ...
            "political_overview": pol,
            "controversies": cont,
            "issues": norm_issues[:25],
            "noteworthy_statements": dedupe_keep(noteworthy)[:8],
            "personal_conflicts": dedupe_keep(conflicts)[:6]
        }
    ...
    def _normalize_analysis(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Normalize people
        raw_people = to_list(analysis.get("people"))
        # Strip non-word chars, collapse spaces
        people = [re.sub(r"[^\w'\-\s]", "", p).strip() for p in raw_people]
        people = [re.sub(r"\s{2,}", " ", p) for p in people]
        # Remove titles/roles often prepended
        title_stop = {"mr", "mrs", "ms", "miss", "dr", "prof", "sir", "madam", "president", "pm", "pres", "senator", "sen", "rep", "representative", "gov", "governor", "minister", "king", "queen", "prince", "princess", "judge", "chairman", "chancellor"}
        def strip_titles(name: str) -> str:
            parts = [w for w in name.split() if w]
            while parts and parts[0].lower().strip(".") in title_stop:
                parts.pop(0)
            return " ".join(parts)
        people = [strip_titles(p) for p in people]
        # Filter obvious non-names / pronouns / placeholders
        non_names = {"i","me","you","he","she","we","they","him","her","us","them","my","your","our","their","mine","yours","ours","theirs",
                     "someone","somebody","everyone","everybody","anyone","anybody","noone","nobody",
                     "guy","guys","dude","bro","man","folks","people","chat","viewers","the speaker","speaker","host","guest","__"}
        people = [p for p in people if len(p) >= 2 and not re.search(r"\d", p) and p.lower() not in non_names]
        ...
        if self.category_map:
            mapped = []
            for c in categories:
                canon = self.category_map.get(c.strip().lower())
                mapped.append(canon or c)
            categories = mapped
...
class AnalysisAggregator:
    ...
    def aggregate(chunk_analyses: List[Dict[str, Any]], metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Merge single-token last names into the most frequent multi-word name sharing that last token
        singles = {n: c for n, c in people_count.items() if len(n.split()) == 1}
        multis = {n: c for n, c in people_count.items() if len(n.split()) >= 2}
        last_to_multi: dict[str, str] = {}
        # Pick the most frequent multi-word per last token
        for n, c in multis.items():
            last = n.split()[-1]
            cur = last_to_multi.get(last)
            if not cur or multis.get(cur, 0) < c:
                last_to_multi[last] = n
        # Reassign counts
        for n, c in list(singles.items()):
            target = last_to_multi.get(n)
            if target:
                people_count[target] = people_count.get(target, 0) + c
                people_count.pop(n, None)
...
def generate_markdown_report(analysis: Dict[str, Any], output_path: Path, hot_target_summary: List[Dict[str, Any]] | None = None):
    ...
    # Hot target overview
    if hot_target_summary:
        md.append(f"\n## Hot Targets")
        for tgt in hot_target_summary:
            md.append(f"- **{tgt.get('target')}**: {tgt.get('spans', 0)} span(s) across {tgt.get('chunks', 0)} chunk(s)")
            examples = tgt.get('examples') or []
            if examples:
                md.append("  - examples:")
...
def main():
    ...
    parser.add_argument('--model', default='granite3.3:8b', help='Ollama model to use')
