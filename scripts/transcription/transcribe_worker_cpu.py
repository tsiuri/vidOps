#!/usr/bin/env python3
"""
transcribe_worker_cpu.py - CPU transcription worker

Standalone transcription worker for CPU using faster-whisper.
Designed to work alongside GPU workers with reduced resource usage.

Usage:
    transcribe_worker_cpu.py --queue-dir <path> --model <size> \\
        --language <code>

Environment Variables:
    See dual_gpu_transcribe.sh for complete list of variables.
    Key vars: FORCE, OUTFMT, CPU_COMPUTE, CPU_THREADS, INLINE_RETRY
...
def _build_log_prefix() -> str:
    ...
    if env_label:
        lab = env_label.strip("[]")
        return f"[{lab}]"
    num = os.environ.get("WORKER_NUM")
    if num and num.isdigit():
        return f"[CP{int(num):02d}]"
    return "[CPU]"
...
def main():
    parser = argparse.ArgumentParser(
        description="CPU transcription worker using faster-whisper"
    )
    ...
    COMPUTE = os.environ.get("CPU_COMPUTE", "int8_float32")
    ...
    print(f"{LOG_PREFIX} CPU threads set to {CPU_THREADS_VAL} (CPU worker needs many threads)", flush=True)
    print(
        f"{LOG_PREFIX} loading faster-whisper model={MODEL} device=cpu "
        f"compute={COMPUTE} threads={CPU_THREADS} num_workers={CPU_THREADS}",
        flush=True
    )
    model = WhisperModel(
        MODEL,
    ...
    while True:
        ...
        # CPU worker uses simpler naming (no model tags)
        ...
        try:
            ...
            if force_fragment or total > FRAGMENTED_THRESHOLD:
                ...
                try:
                    ...
                    run_fragmented_transcription(
                        ...
                        tag=None,
                        skip_lock=True,  # caller holds lock already
                    )
            ...
            try:
                ...
                # CPU worker uses VAD_FILTER=True by default (same as GPU)
            ...
            if low_conf_indices and INLINE_RETRY == 0:
                ...
                # Inline retry for low-confidence segments
            ...
            # Write words.tsv with confidence scores
            ...
            if do_srt:
                ...
                cap = cap or out_srt
            if cap:
                make_tslog(cap, base.with_suffix(".tslog.txt"), MIN_TS)
