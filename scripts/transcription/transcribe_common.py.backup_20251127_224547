...
def split_audio_into_chunks(
    ...
    """
    Split long audio file into chunks to reduce memory usage during transcription.

    For files longer than chunk_duration * 1.5, splits into chunk_duration segments.
    Chunks are stored in /tmp for automatic cleanup.

    Args:
        media_path: Path to media file
        chunk_duration: Duration of each chunk in seconds (default: 3600 = 1 hour)
        log_prefix: Logging prefix (e.g., "[NV0]")

    Returns:
        List of (chunk_path, start_time, temp_dir) tuples.
        - chunk_path: Path to chunk file (in /tmp if chunked, original if not)
        - start_time: Offset in seconds from start of original file
        - temp_dir: Temporary directory path (None if not chunked, caller should clean up)

        If file doesn't need chunking, returns [(media_path, 0.0, None)]
    """
    total_duration = probe_duration_seconds(media_path)

    if not total_duration:
        # Can't determine duration, process as-is
        return [(media_path, 0.0, None)]

    # Don't chunk if only slightly over threshold (avoid tiny last chunk)
    if total_duration <= chunk_duration * 1.5:
        return [(media_path, 0.0, None)]

    print(
        f"{log_prefix}[CHUNK] File is {total_duration/3600:.1f}h, splitting into "
        f"{chunk_duration/60:.0f}min chunks to reduce memory usage",
        flush=True
    )

    # Create temp directory for chunks in /tmp
    temp_dir = tempfile.mkdtemp(prefix="transcribe_chunks_", dir="/tmp")

    try:
        chunks = []
        num_chunks = int((total_duration + chunk_duration - 1) // chunk_duration)  # Ceiling division

        for i in range(num_chunks):
            start_time = i * chunk_duration
            chunk_path = Path(temp_dir) / f"chunk_{i:04d}.wav"
            ...
            chunks.append((chunk_path, start_time, temp_dir))

            print(
                f"{log_prefix}[CHUNK] Created chunk {i+1}/{num_chunks} "
                f"(starts at {start_time/3600:.2f}h, duration {chunk_duration/60:.0f}min)",
                flush=True
            )
