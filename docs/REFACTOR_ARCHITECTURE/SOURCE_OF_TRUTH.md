...
- **Analysis scheduling (VRAM):** ✅ UPDATED. Analysis task claiming now uses `required_vram_gb` and model profiles (`analysis_model_profiles`) to gate work by available VRAM. Each profile represents a model + options tuple, keyed by numeric `id`, and is referenced via `model_profile_id`. Legacy capability tags remain in the DB for potential future use but no longer affect analysis task scheduling.
