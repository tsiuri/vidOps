...
- **Storage/Cache Manager:** ✅ IMPLEMENTED. FilesystemCache provides two-tier storage (central + local cache), pull_to_cache(), asset registration, path resolution. Documented in `docs/STORAGE_INTERFACE.md`. Integrated into DownloadService. Ready for other services to use.
  - Cross-platform path mapping: `paths.path_map` in `config.yaml` can translate absolute DB paths (e.g., `/mnt/13tb_sas/...`) into local OS paths (UNC/drive paths on Windows); prefer `assets.rel_path` for portable paths.
  - Repo line endings are enforced to LF via `.gitattributes` to keep Linux/Windows checkouts consistent.
...
- **Diarization:** ✅ NATIVE PYANNOTE. `DiarizationService` + `GenericWorker` stage media in `pull/`, words in `generated/`, references under `data/references/<ytid>/`, run `scripts/diarization/batch_diarize.py` (chunked when needed), and register `diarization` assets (`diarized_timestamps.tsv`, `speaker_words.tsv`, `diarization.json`) under `generated/diarization_resemblyzer/<ytid>/`; fake mode via `VIDOPS_FAKE_DIARIZATION`. Diarization parameters (device, chunk_seconds, thresholds, etc.) default to `config.yaml` (`diarization.*` block) with CLI overrides. `DiarizeWorker` exits cleanly on SIGINT/SIGTERM by releasing the job back to PENDING and terminating spawned diarization subprocesses.
  - Windows note: `scripts/diarization/chunk_audio.py` forces UTF-8 stdout/stderr to avoid cp1252 UnicodeEncodeError when filenames include non-ASCII characters.
  - Windows note: Memory monitor uses username-based filtering when `os.getuid` is unavailable (avoids repeated warnings during diarization).
...
- **Legacy stitch/analyze/dates/extra-utils:** ✅ BRIDGED. Stitch/analyze/dates now run native paths with legacy layouts; extra-utils remains Linux-only (workspace.sh) and is gated on Windows.
...
- **Orchestration:** ✅ AUTOMATED. Two approaches available:
  1. **Overlord-driven chaining**: Polls for completed transcription jobs and auto-enqueues analysis (legacy flow, **currently disabled**)
...
- **Monitoring CLI:** ✅ UPDATED. `vo_cli.py status workers` and `status jobs` hit the unified tables, expose heartbeat ages/stale counts, and support per-`job_type` breakdowns.
  - `vo worker stop` terminates local workers by worker_id or machine alias (kills by PID; use --all/--force).
- **Worker heartbeats:** ✅ UPDATED. All workers run a shared background heartbeat that updates `workers.last_heartbeat` and touches active jobs' `updated_at`; analysis-distributed now registers in the `workers` table.
- **Clips/Stitch CLI:** ✅ UPDATED. `vo_cli.py clips hits/cut` replaces the legacy workspace flow (DAL-backed phrase search + manifest-driven enqueue). `clips cut` is manifest-level (one job per TSV), defaults to `--mode net`, supports `--mode local`, and outputs under `generated/hits/<run_name>/` using native ffmpeg/yt-dlp (legacy filenames preserved). `vo_cli.py stitch enqueue` handles manifest-based concatenation jobs.
...
- **Analysis scheduling (VRAM):** ✅ UPDATED. Analysis task claiming uses `required_vram_gb` and model profiles (`analysis_model_profiles`) to gate work by available VRAM. Each profile represents a model + options tuple, keyed by numeric `id`, and is referenced via `model_profile_id`. **Legacy capabilities system was removed** (Migration 007, 2025-12-31) - the `required_capabilities` and `capabilities` columns have been dropped from `analysis_tasks` and `workers` tables respectively.
...
- GenericWorker runs cleanup when the workspace exceeds `workspace.tmp_cleanup.trigger_workspace_size_gb` at startup, and again during heartbeat size checks if tmp/ or the workspace exceeds their limits. The cleanup sweeps only known heavy dirs (`tmp/raw`, `tmp/generated`, diarization chunk dirs, reference_builder scratch, `tmp/mnt`, pull/, generated/) for files above `min_bytes` and older than `min_age_minutes`, skips small-text extensions, caps deletions, logs each removal to `logs/cleanup.log`, and uses a lock file to avoid concurrent workers. At startup, if still above the limit after cleanup, the worker prompts (default N) before continuing; during heartbeat checks, it will retry cleanup once and terminate if still over the limit.
...
- **Exception:** Transcription now runs via native faster-whisper (no `workspace.sh`); it writes VTT/words locally and persists through FilesystemCache/DB ingestion without shelling to legacy scripts.
...
- **clips hits / cut-local / cut-net / refine** (native): Rebuild the TSV/manifest files under `generated/hits/<run>/`, run native ffmpeg (cut-local) or yt-dlp section downloads (cut-net) with the legacy filename template, then register clip assets (hits TSV, raw clips, refined clips) and push them to storage before completing the job.
- **dl-subs** (native yt-dlp): Write subtitles into `pull/` using legacy patterns (`<ytid>__<title>.transcript.<lang>.vtt`), then register subtitle/transcript assets (with `rel_path`) and push them to storage via FilesystemCache; mark the job complete only after DB ingestion.
- **voice** (native): Stage reference clips/target clips in legacy paths, run native voice filter scripts (`filter_voice_parallel_chunked.py` et al.), emit `voice_analysis.json` and `hasan_clips.txt`, then ingest matched lists/JSON into the DB as assets and push outputs to storage before closing the job.
- **diarize** (native pyannote): Pull media into `pull/`, words into `generated/`, references into `data/references/<ytid>/`, run `scripts/diarization/batch_diarize.py` (chunked when needed), emit `diarized_timestamps*.tsv`, `speaker_words.tsv`, `diarization.json` under `generated/diarization_resemblyzer/<ytid>/`, then insert spans into DB and push artifacts to storage.
- **transcribe** (native Whisper): Pull media via FilesystemCache, run faster-whisper directly (word timestamps + VAD option), write VTT + words TSV into legacy layout as needed, ingest words into DB, register VTT/words assets, and push artifacts to storage; no `workspace.sh` dependency.
...
- **extra-utils** (`workspace.sh extra-utils …`): Linux-only; wrapped jobs are rejected on Windows. When enabled on Linux, rebuild inputs, run legacy tool, ingest outputs, push to storage, then mark the job complete.
...
- Dependency note (2026-01-01): `requirements.txt` now uses platform markers for CUDA stacks. Linux stays on torch/torchaudio 2.8.0+cu128 with the full `nvidia-*` wheels (Python 3.13). Windows uses torch/torchaudio/torchvision 2.4.1+cu121 on Python 3.12, with pyannote-audio 3.1.1 (Linux uses pyannote-audio 4.0.2). WebRTC VAD uses webrtcvad on non-Windows and webrtcvad-wheels on Windows; Windows also installs a local webrtcvad shim wheel to satisfy dependency resolution. CUDA-only wheels (nvidia-*, triton) are gated to Linux; `audioop-lts` is Linux-only to avoid missing Windows wheels.
