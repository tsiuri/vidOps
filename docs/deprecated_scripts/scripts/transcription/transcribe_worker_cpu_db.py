#!/usr/bin/env python3
"""
transcribe_worker_cpu_db.py - CPU worker with database queue support

Enhanced version of transcribe_worker_cpu.py that supports both:
- Traditional file-based queue (--queue-dir)
- Database queue (USE_DB_QUEUE=1)

Usage:
    # Database queue mode (new)
    USE_DB_QUEUE=1 python3 transcribe_worker_cpu_db.py --model medium

    # File-based queue mode (legacy)
    python3 transcribe_worker_cpu_db.py --queue-dir /path/to/queue --model medium
...
Environment Variables:
    ...
    All existing transcribe_worker_cpu.py environment variables still work:
    FORCE, OUTFMT, CPU_COMPUTE, CPU_THREADS, INLINE_RETRY, etc.

Author: Adapted from transcribe_worker_nvidia_db.py (2025-11-28)
"""
...
def run_file_based_queue(args, model, config):
    ...
    while True:
        task = claim_task(QDIR, backend="cpu")
        if not task:
            try:
...
def run_database_queue(args, model, config):
    ...
    class CpuDBWorker(QueueWorkerBase):
        def __init__(self, model_inst, config_dict, **kwargs):
            super().__init__(
                worker_type="cpu",
                gpu_index=None,
        ...
        def _process_job(self, job: dict):
            ...
            try:
                ...
                if result:
                    ...
                    # File was skipped
                    print(f"{LOG_PREFIX} Job {job_id} skipped, output already exists.", flush=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="CPU transcription worker (file or database queue)"
    )
    ...
    COMPUTE = os.environ.get("CPU_COMPUTE", "int8")
    ...
    print(f"{LOG_PREFIX} Loading model={MODEL} device=cpu compute={COMPUTE} threads={CPU_THREADS_VAL} num_workers={CPU_THREADS_VAL}", flush=True)
    start_load = time.time()
    model = WhisperModel(MODEL, device="cpu", compute_type=COMPUTE, cpu_threads=CPU_THREADS_VAL, num_workers=CPU_THREADS_VAL)
